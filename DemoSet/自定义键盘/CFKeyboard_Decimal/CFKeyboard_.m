...
//  customkeyboard
//
//  Created by Mountain.Cao on 17/7/20.
//  Copyright © 2016年 Mountain.Cao All rights reserved.
//

#import "CFKeyboard_.h"

static UIImage *cfdk_image(UIColor *color, CGSize size)
{
...
//删除键
static UIImage *cfdk_delete_icon()
{
...
//键盘图标
static UIImage *cfdk_keyboard_icon()
{
    CGFloat scale = [UIScreen mainScreen].scale;
...
@interface UIView (CFDKFindFirstResponder)

+ (UIView *)cfdk_firstResponder;

@end


@implementation UIView (CFDKFindFirstResponder)

+ (UIView *)cfdk_firstResponder
{
    return [[[UIApplication sharedApplication] keyWindow] cfdk_findFirstResponder];
}

- (UIView *)cfdk_findFirstResponder
{
    if ([self isFirstResponder]) return self;
    
    for (UIView *view in self.subviews) {
        UIView *responder = [view cfdk_findFirstResponder];
...
@class CFDecimalKeyboardItem;

@interface CFDecimalKeyboardItem : UIButton
...
@implementation CFDecimalKeyboardItem



- (void)setBackgroundColor:(UIColor *)backgroundColor forState:(UIControlState)state
{
    [self setBackgroundImage:cfdk_image(backgroundColor, CGSizeMake(1, 1))
                    forState:state];
}
...
@interface CFKeyboard_ () <UIInputViewAudioFeedback>
{
    NSTimer *deleteTimer;
}
...
@property (nonatomic, assign) CFKeyboardType    keyboardType;

@property (nonatomic, assign) BOOL      showButton;

@property (nonatomic, copy) HandleEvent handleEventBlock;
...
@property (nonatomic, copy) HandleEvent compelementBlock;

@end


@implementation CFKeyboard_
...
+ (instancetype)keyboard {
    ...
    return [self keyboardWithCompelement:nil];
}

+ (instancetype)keyboardWithCompelement:(HandleEvent)compelementBlock {
    
    return [self keyboardType:CFKeyboardTypeDecimal compelement:compelementBlock];
}

+ (instancetype)keyboardType:(CFKeyboardType)keyboardType compelement:(HandleEvent _Nullable)compelementBlock {
    
    return [self keyboardType:keyboardType buttonTitle:nil buttonClick:nil compelement:compelementBlock];
}
...
+ (instancetype)keyboardType:(CFKeyboardType)keyboardType buttonTitle:(NSString * _Nullable)title buttonClick:(HandleEvent _Nullable)buttonBlock compelement:(HandleEvent _Nullable)compelementBlock {
    ...
    return [self keyboardType:keyboardType buttonTitle:title buttonAttributes:nil buttonClick:buttonBlock compelement:compelementBlock];
}
...
+ (instancetype)keyboardType:(CFKeyboardType)keyboardType buttonTitle:(NSString * _Nullable)title buttonAttributes:(NSDictionary<NSString *,id> * _Nullable)attributes buttonClick:(HandleEvent _Nullable)buttonBlock compelement:(HandleEvent _Nullable)compelementBlock {
    ...
    return [self keyboardType:keyboardType buttonTitle:title buttonAttributes:attributes buttonBackgroundColor:[UIColor redColor] buttonClick:buttonBlock compelement:compelementBlock];
}
...
+ (instancetype)keyboardType:(CFKeyboardType)keyboardType buttonTitle:(NSString * _Nullable)title buttonAttributes:(NSDictionary<NSString *,id> * _Nullable)attributes buttonBackgroundColor:(UIColor * _Nullable)backgroundColor buttonClick:(HandleEvent _Nullable)buttonBlock compelement:(HandleEvent _Nullable)compelementBlock {
    ...
    return [[self alloc] keyboardType:keyboardType buttonTitle:title buttonAttributes:attributes buttonBackgroundColor:backgroundColor buttonClick:buttonBlock compelement:compelementBlock];
}

- (instancetype)keyboardType:(CFKeyboardType)keyboardType buttonTitle:(NSString * _Nullable)title buttonAttributes:(NSDictionary<NSString *,id> * _Nullable)attributes buttonBackgroundColor:(UIColor * _Nullable)backgroundColor buttonClick:(HandleEvent _Nullable)buttonBlock compelement:(HandleEvent _Nullable)compelementBlock {
    self.keyboardType = keyboardType;
    self.buttonTitle = title;
    self.buttonBgColor = backgroundColor;
    self.attributes = attributes;
    self.handleEventBlock = buttonBlock;
    self.compelementBlock = compelementBlock;
    self.showButton = (self.buttonTitle.length>0);
    return [self init];
}

- (instancetype)init {
    
    CGRect rect = [self cf_get_frame];
...
- (CGRect) cf_get_frame
{
    ...
    CGFloat height = kCFKeyboardHeight;
    
    if (self.showButton) {
        
        height = kCFKeyboardWithButtonHeight;
    }
...
- (void)_initUI
{
    ...
    if (self.showButton) {
        ...
        CFDecimalKeyboardItem *investItem = [CFDecimalKeyboardItem buttonWithType:UIButtonTypeCustom];
        investItem.tag = 100;
        [investItem setTitle:self.buttonTitle forState:UIControlStateNormal];
        [investItem setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        investItem.titleLabel.font = [UIFont systemFontOfSize:15];
        [investItem setBackgroundColor:self.buttonBgColor forState:UIControlStateNormal];
        [investItem setBackgroundColor:self.buttonBgColor forState:UIControlStateHighlighted];
        
        if (self.attributes) {
            if (self.attributes[NSForegroundColorAttributeName]) {
                [investItem setTitleColor:self.attributes[NSForegroundColorAttributeName] forState:UIControlStateNormal];
            }
            if (self.attributes[NSFontAttributeName]) {
                investItem.titleLabel.font = self.attributes[NSFontAttributeName];
            }
        }
        [self addSubview:investItem];
        [investItem addTarget:self action:@selector(investClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    
    for (int i = 0; i < 14; ++i) {
        
        CFDecimalKeyboardItem *item = [CFDecimalKeyboardItem buttonWithType:UIButtonTypeCustom];
        
        if (i == 10) {
            item.tag = 46;
            
            if (self.keyboardType == CFKeyboardTypeNumber) {
                [item setTitle:@"" forState:UIControlStateNormal];
            } else if (self.keyboardType == CFKeyboardTypeID) {
                [item setTitle:@"X" forState:UIControlStateNormal];
            } else if (self.keyboardType == CFKeyboardTypeDecimal) {
                [item setTitle:@"." forState:UIControlStateNormal];
            }
        }
        else if (i == 11) {
            item.tag = 127;
            [item setImage:cfdk_delete_icon() forState:UIControlStateNormal];
            ...
            item.tag = -1;
            [item setImage:cfdk_keyboard_icon() forState:UIControlStateNormal];
        }
        else if (i == 13) {
            item.tag = -2;
        ...
        [item setTitleColor:[UIColor colorWithRed:3 / 255.f
                                            green:3 / 255.f
                                             blue:3 / 255.f
                                            alpha:1] forState:UIControlStateNormal];
        
        [item.titleLabel setFont:[UIFont fontWithName:@"HelveticaNeue" size:24]];
...
- (void)_clickedDot
{
    if (self.keyboardType == CFKeyboardTypeDecimal) {
        [self _insert:@"."];
    } else if (self.keyboardType == CFKeyboardTypeID) {
        [self _insert:@"X"];
    }
...
{
    [self.firstResponder resignFirstResponder];
    !self.compelementBlock ?: self.compelementBlock();
}
...
- (void)investClick:(UIButton *)button {
...
- (void)layoutSubviews
{
    ...
    if (self.showButton) {
        topMargin = 44;
        height = (CGRectGetHeight(self.bounds)-topMargin) / 4.f - .5f;
        [self viewWithTag:100].frame = CGRectMake(0, 0, CGRectGetWidth(self.bounds), topMargin);
    }
    
    //14个按键
    //1
    [self viewWithTag:49].frame = CGRectMake(0, topMargin+.5f, width, height);
    
    //2
    [self viewWithTag:50].frame = CGRectMake(width + .5f, topMargin+.5f, width, height);

    //3
    [self viewWithTag:51].frame = CGRectMake((width + .5f) * 2, topMargin+.5f, width, height);
    
    //4
    [self viewWithTag:52].frame = CGRectMake(0, topMargin+height + 1.f, width, height);

    //5
    [self viewWithTag:53].frame = CGRectMake(width + .5f, topMargin+height + 1.f, width, height);

    //6
    [self viewWithTag:54].frame = CGRectMake((width + .5f) * 2, topMargin+height + 1.f, width, height);
    
    //7
    [self viewWithTag:55].frame = CGRectMake(0, topMargin +.5f + (height + .5f) * 2, width, height);
    
    //8
    [self viewWithTag:56].frame = CGRectMake(width + .5f,  topMargin +.5f + (height + .5f) * 2, width, height);
    
    //9
    [self viewWithTag:57].frame = CGRectMake((width + .5f) * 2, topMargin + .5f + (height + .5f) * 2, width, height);
    
    //0
    [self viewWithTag:48].frame = CGRectMake(width + .5f, topMargin + .5f + (height + .5f) * 3, width, height);
    
    //.
    [self viewWithTag:46].frame = CGRectMake(0, topMargin + .5f + (height + .5f) * 3, width, height);

    //取消键盘
    [self viewWithTag:-1].frame = CGRectMake((width + .5f) * 2, topMargin + .5f + (height + .5f) * 3, width, height);
    // x
    [self viewWithTag:127].frame = CGRectMake((width + .5f) * 3, topMargin + .5, width, height * 2 + .5f);

    //确定
    [self viewWithTag:-2].frame = CGRectMake((width + .5f) * 3, topMargin+1.5f + height * 2, width, height * 2 + .5f);

}

#pragma mark - Action

- (void)buttonTouchUpInside:(CFDecimalKeyboardItem *)sender
{
    if (!self.firstResponder || ![self.firstResponder isFirstResponder]) {
        self.firstResponder = (UIView<UIKeyInput>*)[UIView cfdk_firstResponder];
