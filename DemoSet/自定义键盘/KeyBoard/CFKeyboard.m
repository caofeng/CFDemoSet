...
#define CFKBH                           216
#define CFICOCF                         35
#define CFCHAR_CORNER                   8
#define CFKBFontSize                    18
#define CFKBFont(s)                        [UIFont fontWithName:@"HelveticaNeue-Light" size:s]
#define CFFormat(format, ...)           [NSString stringWithFormat:format, ##__VA_ARGS__]
#ifndef CFSCREEN_WIDTH
#define CFSCREEN_WIDTH [UIScreen mainScreen].bounds.size.width
#endif

@interface CFKeyboard ()

@property (nonatomic, assign) CFKBType type;
...
static UIColor *CFColor(int r, int g, int b) {
    return [UIColor colorWithRed:r/255.f green:g/255.f blue:b/255.f alpha:1.f];
}

@implementation CFKeyboard

+ (nullable)keyboardWithType:(CFKBType)type {
    return [[CFKeyboard alloc] initWithFrame:CGRectMake(0, 0, CFSCREEN_WIDTH, CFKBH+CFICOCF) withType:type];
}

- (id)initWithFrame:(CGRect)frame withType:(CFKBType)type {
...
- (CGFloat)widthForInfo:(NSString *)info {
    ...
    NSDictionary *attributs = [NSDictionary dictionaryWithObjectsAndKeys:CFKBFont(CFKBFontSize-5), NSFontAttributeName, nil];
...
- (void)setEnterprise:(NSString *)enterprise {
    if (enterprise.length > 0) {
        ...
        CGFloat icoCF = CFICOCF;
        CGFloat icon_w_h = icoCF*0.5;
        CGFloat width = [self widthForInfo:enterprise];
        CGFloat wwww = icon_w_h*1.5+width;
        CGFloat start_x = (CFSCREEN_WIDTH-wwww)*0.5+icon_w_h*1.5;
        CGRect bounds = CGRectMake(start_x, CFICOCF*0.25, width, CFICOCF*0.5);
...
- (UILabel *)iconLabel {
    if (!_iconLabel) {
        ...
        _iconLabel.font = CFKBFont(CFKBFontSize-5);
        _iconLabel.textColor = CFColor(174, 174, 174);
...
- (UILabel *)nextLabel {
    if (!_nextLabel) {
        ...
        _nextLabel = [[UILabel alloc]initWithFrame:CGRectMake(CFSCREEN_WIDTH-80, 0, 80, 34)];
...
- (UIImageView *)iconFlag {
    if (!_iconFlag) {
        ...
        _iconFlag.layer.cornerRadius = CFICOCF*0.25;
...
- (void)_initSetup {
    //默认信息
    self.icon = @"58";
    self.enterprise = @"中业兴融安全键盘";
    self.next = @"完成";
    CGFloat icoCF = CFICOCF;
    CGFloat kbH = CFKBH;
    UIColor *bgColor = CFColor(124, 124,124);
    CGRect bounds = self.bounds;
    bounds.size.height = kbH+icoCF;
    self.bounds = bounds;
    self.backgroundColor = bgColor;
    //seperate line
    CGFloat lineH = 1;
    bounds = CGRectMake(0, CFICOCF-lineH, CFSCREEN_WIDTH,lineH);
    
    UIView *line = [[UIView alloc] initWithFrame:bounds];
    line.backgroundColor = CFColor(101, 101, 101);
    [self addSubview:line];
    
    //创建键盘
    if (CFKBTypeNumberPad == self.type) {
        [self reloadNumberPad:false];
    }else if (CFKBTypeDecimalPad == self.type){
        [self reloadNumberPad:true];
    }else if (CFKBTypeASCIICapable == self.type){
        [self setupASCIICapableLayout:true];
    }else if (CFKBTypeIDNumberPad == self.type) {
        [self reloadNumberPad:true];
...
- (void)willMoveToWindow:(UIWindow *)newWindow {
    ...
    if (!newWindow && self.type != CFKBTypeASCIICapable) {
        [self loadRandomNumber];
    }
}

- (void)nextStepClick:(UITapGestureRecognizer *)tap {
    
    if (self.inputSource) {
        if ([self.inputSource isKindOfClass:[UITextField class]]) {
            UITextField *tmp = (UITextField *)self.inputSource;
            if (tmp.delegate && [tmp.delegate respondsToSelector:@selector(textFieldShouldReturn:)]) {
                BOOL ret = [tmp.delegate textFieldShouldReturn:tmp];
                [tmp endEditing:ret];
            }else{
...
- (void)reloadNumberPad:(BOOL)decimal {
    if (self.type != CFKBTypeASCIICapable) {
        
        int cols = 3;
        int rows = 4;
        UIColor *lineColor = CFColor(101, 101, 101);
        UIColor *titleColor = CFColor(247, 247, 247);
        UIColor *touchColor = CFColor(250, 250, 250);
        UIFont *titleFont = CFKBFont(CFKBFontSize);
        CGFloat itemH = CFKBH/rows;
        CGFloat itemW = CFSCREEN_WIDTH/cols;
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                CGRect bounds = CGRectMake(j*itemW, i*itemH+CFICOCF, itemW, itemH);
...
#pragma mark -- 数字键盘随机数产生

- (void)loadRandomNumber {
    
    BOOL random = YES;//设置是否需要随机
    
    BOOL decimal = (self.type == CFKBTypeDecimalPad || self.type == CFKBTypeIDNumberPad);
    
    NSArray *titles = nil;
    
    if (random) {
        
        if (self.type == CFKBTypeDecimalPad) {
            titles = [self generateRandomNumberWithDecimal:YES];
        } else if (self.type == CFKBTypeIDNumberPad) {
            titles = [self generateRandomNumberWithIDCard:YES];
        } else if (self.type == CFKBTypeNumberPad) {
            titles = [self generateRandomNumberWithDecimal:NO];
        }
...
- (void)touchDownAction:(UIButton *)btn {
    UIColor *touchColor = CFColor(43, 116, 224);
...
- (NSArray *)generateRandomNumberWithIDCard:(BOOL)idCard {
    NSMutableArray *tmp = [NSMutableArray array];
    for (int i = 0; i < 10; i++) {
        NSString *c = CFFormat(@"%d",i);
        [tmp addObject:c];
...
- (void)setupASCIICapableLayout:(BOOL)init {
    ...
    if (!init) {
        ...
        NSArray *subviews = self.subviews;
        [subviews enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[CFChar class]]) {
    ...
    CGFloat char_width = (CFSCREEN_WIDTH-char_h_dis*len)/len;
    CGFloat char_heigh = (CFKBH-char_uper_dis*2-char_v_dis*3)/4;
    UIFont *titleFont = CFKBFont(CFKBFontSize);
    UIColor *titleColor = CFColor(247, 247, 247);
    UIColor *bgColor = CFColor(64, 66, 68);
    UIImage *bgImg = [UIImage pb_imageFromColor:bgColor];
    CGFloat cur_y = CFICOCF+char_uper_dis;
    
    int n = 0;
    UIImage *charbgImg = [bgImg pb_drawRectWithRoundCorner:CFCHAR_CORNER toSize:CGSizeMake(char_width, char_heigh)];
    for (int i = 0 ; i < len; i ++) {
        ...
        CFChar *btn = [CFChar buttonWithType:UIButtonTypeCustom];
        btn.frame = bounds;
        //btn.chars = title;
        btn.exclusiveTouch = true;
        //消耗性能
    ...
    //NSLog(@"第二排:%@",chars);
    len = [chars count];
    CGFloat start_x = (CFSCREEN_WIDTH-char_width*len-char_h_dis*(len-1))/2;
    for (int i = 0 ; i < len; i ++) {
        ...
        btn.exclusiveTouch = true;
        //[btn addRoundCornerBackdround];
        //btn.backgroundColor = CFColor(64, 66, 68);
        btn.userInteractionEnabled = false;
        btn.titleLabel.font = titleFont;
        btn.titleLabel.textAlignment = NSTextAlignmentCenter;
        btn.titleLabel.textColor = titleColor;
    ...
    char_width = (CFSCREEN_WIDTH-char_h_dis*4-shiftWidth*2-char_h_dis*(len-1))/len;
    charbgImg = [bgImg pb_drawRectWithRoundCorner:CFCHAR_CORNER toSize:CGSizeMake(char_width, char_heigh)];
    CGRect bounds;UIButton *btn;
    if (init) {
        UIImage *roundImg = [bgImg pb_drawRectWithRoundCorner:CFCHAR_CORNER toSize:CGSizeMake(shiftWidth, char_heigh)];
        ...
        bounds = CGRectMake(CFSCREEN_WIDTH-char_h_dis*0.5-shiftWidth, cur_y, shiftWidth, char_heigh);
        btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = bounds;
        btn.exclusiveTouch = true;
        //btn.backgroundColor = CFColor(64, 66, 68);
    ...
    if (init) {
        ...
        UIImage *roundImg = [bgImg pb_drawRectWithRoundCorner:CFCHAR_CORNER toSize:CGSizeMake(numWidth, char_heigh)];
        ...
        bounds = CGRectMake(CFSCREEN_WIDTH-char_h_dis*0.5-numWidth, cur_y, numWidth, char_heigh);
        btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = bounds;
        btn.exclusiveTouch = true;
        //btn.backgroundColor = CFColor(64, 66, 68);
        ...
        CGFloat spaceWidth = (CFSCREEN_WIDTH-char_h_dis*3-numWidth*2);
...
- (void)loadCharacters:(NSArray *)array {
    ...
    NSArray *subviews = self.subviews;
    [subviews enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj && [obj isKindOfClass:[CFChar class]]) {
            CFChar *tmp = (CFChar *)obj;
            NSInteger __tag = tmp.tag;
...
- (void)characterTouchAction:(CFChar *)btn {
    NSString *title = [btn titleLabel].text;
    if (self.inputSource) {
        if ([self.inputSource isKindOfClass:[UITextField class]]) {
            UITextField *tmp = (UITextField *)self.inputSource;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    for (CFChar *tmp in self.charsBtn) {
...
@implementation CFChar

- (void)addRoundCornerBackdround {
    CGSize size = [self bounds].size;
    UIImage *backImg = [UIImage pb_imageFromColor:CFColor(64, 66, 68)];
    backImg = [backImg pb_drawRectWithRoundCorner:CFCHAR_CORNER toSize:size];
...
- (void)addPopup {
    ...
    if ([self.chars isEqualToString:@"q"]||[self.chars isEqualToString:@"a"]) {
        keyPop = [[UIImageView alloc] initWithImage:[UIImage imageWithCGImage:[self createKeytopImageWithKind:CFKBImageRight] scale:[[UIScreen mainScreen] scale] orientation:UIImageOrientationDown]];
        ...
        keyPop = [[UIImageView alloc] initWithImage:[UIImage imageWithCGImage:[self createKeytopImageWithKind:CFKBImageLeft] scale:[[UIScreen mainScreen] scale] orientation:UIImageOrientationDown]];
        keyPop.frame = CGRectMake(-38, -71, keyPop.frame.size.width, keyPop.frame.size.height);
    }
    else {
        keyPop = [[UIImageView alloc] initWithImage:[UIImage imageWithCGImage:[self createKeytopImageWithKind:CFKBImageInner] scale:[[UIScreen mainScreen] scale] orientation:UIImageOrientationDown]];
        keyPop.frame = CGRectMake(-27, -71, keyPop.frame.size.width, keyPop.frame.size.height);
    }
    NSString *tmp = self.isShift?[self.chars uppercaseString]:[self.chars lowercaseString];
    [text setFont:[UIFont fontWithName:CFKBFont(CFKBFontSize).fontName size:44]];
    [text setTextColor: CFColor(247, 247, 247)];
